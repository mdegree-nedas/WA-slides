...
\addcontentsline{toc}{subsection}{algoritmo Progr-Dyn-Knapsack: approccio duale}
\begin{flushleft}
	\begin{itemize}
		\item definizione:
		\begin{itemize}
	...
	\begin{algorithm}
		...
		\begin{algorithmic}
			...
			\FOR{$i=1$ to $n$}
				...
				\ENDFOR
			\ENDFOR
			\RETURN $\max p\vert V[n,b]\leq b$
		\end{algorithmic}
	\end{algorithm}
	\begin{itemize}
		\item problema: come dovremmo scegliere $P$?
		\item risposta: grande abbastanza da includere l'ottimo, ovvero qualunque $upper bound$ di $m^*$, cio\'e $P\geq m^*$
		\item scelta:
